<template>
    ...
    <div v-if="isLoaded">
        ...
        <div v-for="memo in memos">
            <router-link
                :to="{ path: '/mydraw/public/memo/', query: { id: memo.urlparam } }">{{ memo.memotitle }}</router-link>
        </div>
        <h3>参加したメモ</h3>
...
const logout = async () => {
    ...
    router.push('/mydraw/public/login')
}
...
const newmemo = async () => {
    ...
    router.push({ path: '/mydraw/public/memo/', query: { id: randomString } })
}
